      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {testimonials.map((t) => (
            <div
              key={t.author}
              className="relative p-6 rounded-lg border border-[#1a1a1a] bg-[#070707] border-l-[3px] border-l-green-500/30 hover:border-l-green-500 hover:-translate-y-0.5 hover:shadow-glow-green-sm transition-all duration-300"
            >
              <svg className="w-6 h-6 text-green-500/40 mb-3" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <p className="font-mono text-xs font-bold text-green-500 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_6px_rgba(0,255,0,0.6)]" />
                  {t.author}
                </p>
                <p className="font-mono text-[10px] text-gray-600 mt-1">{t.role}</p>
              </div>
            </div>
          ))}
        </div>
